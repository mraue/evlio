	 ...
	 _extension_version(0)
{
    ...
    int status=0;
    string fullname = fits_filename + "("+template_filename+")";
...
addToMap( string column, Val *colval ) {
    ...
    if ( (colval->getType() != colinfo.typecode )
	 && !(colval->getType()==TULONG  && colinfo.typecode==TLONG )
	 && !(colval->getType()==TLONG  && colinfo.typecode==TULONG ) 
