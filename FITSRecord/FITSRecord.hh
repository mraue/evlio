...
/**
 * A FITSRecord that can read from an existing FITSFile. 
 * For example:
 *
 * \code
 * InputFITSRecord events("events.fits");
 * double energy;
 * double ra,dec;
 *
 ...
 * \endcode
 */
class InputFITSRecord : public FITSRecord
{
public:
    InputFITSRecord(std::string url);
    
    int isFinished(){ return _is_finished; }
    bool read();
