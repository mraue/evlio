...
/**
 * construct a FITSRecord with no initialization
 */
FITSRecord::FITSRecord() 
    : _fptr(NULL),_own_fptr(false), _rowcount(0), _verbose(0),
      _extension_version(0) 
{
...
FITSRecord::FITSRecord( std::string fits_filename, 
    ...
    : 	 _fptr(NULL),_own_fptr(true), _rowcount(0), _verbose(3),
	 _extension_version(0)
{
    ...
    if (_verbose>=2) cout << "Looking for: "<< fits_filename << endl;
    fits_open_file( &_fptr, fits_filename.c_str(), READWRITE, &status );

    if (status) {
	if (_verbose>=2) cout << "REOPENING "<< fits_filename << endl;
	fitsfile *newfptr;
	...
	if (_verbose>=2) cout << "CREATING "<< fullname.c_str() << endl;
...
finishWriting() {
    ...
    fits_update_key( _fptr, TULONG, "NAXIS2", 
		     &_rowcount, "number of events", &status);
    if (status) 
...
locateTemplate( string template_filename ) {
	...
	ifstream infile(template_filename.c_str());
	if (infile) {
	    infile.close();
...
InputFITSRecord::InputFITSRecord( std::string url ) 
    : FITSRecord(), _is_finished(true)
{

    int status=0;

    if (_verbose>=1) cout << "OPENING URL: "<< url << endl;
    fits_open_file( &_fptr, url.c_str(), READONLY, &status );

    if (status) {
	throw FITSRecordError("InputFITSRecord open file  "+url
			      +": "+getFITSError(status));
    }
    ...
    if (_verbose>1) cout << "EXTENSION VERSION: "<<  _extension_version << endl;
 ...
 *
 * \returns true if a data row was read, false if no data is available (or file is finished)
 */
bool
InputFITSRecord::
read() {
